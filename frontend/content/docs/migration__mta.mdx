...
Vital.sandbox runs on **Godot 4 + C++17** with a purpose-built Lua sandbox. The rendering stack, physics engine, networking layer, and asset pipeline are fully configurable. Vital.sandbox is a standalone platform with no dependency on GTA SA or its assets. Asset formats, the resource system, and scripting patterns differ from MTA.
...
| File I/O | `fileOpen` / `fileWrite` / `fileRead` | `util.file` — read, write, hash, delete |
| Networking | MTA sync layer | `ISyncable` net IDs, `core.engine.get_entity_by_net_id`, server-authoritative replication |
| Async scripting | Coroutine-based | `util.thread` + `util.promise` — yield on await, resume on resolution |
