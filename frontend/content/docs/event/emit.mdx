...
<Callout type="info">
  **`options` is positional — pass `nil` or omit it entirely when no routing is needed.**
  ...
  - **Handler invocation order** — handlers are invoked in [`priority`](/docs/event/on#config) order, highest first; handlers sharing the same priority fire in registration order.
</Callout>
