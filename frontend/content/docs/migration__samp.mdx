...
Vital.sandbox uses **Lua** — dynamically typed, first-class functions, coroutines, and closures. The API surface is consistent and structured. Vital.sandbox is a standalone platform with no dependency on GTA SA or its assets. Asset formats, the resource system, and scripting patterns differ from SA-MP.
