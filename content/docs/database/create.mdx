...
```lua
local instance = core.database.create(
    host,
    user,
    password,
    database,
...
```lua lineNumbers title="Connect to a local database without a password"
local entity = core.database.create("127.0.0.1", "root", "", "vital_sandbox")
```

```lua lineNumbers title="Connect with a password and custom port"
local entity = core.database.create("127.0.0.1", "root", "secret", "vital_sandbox", 3307)
```
