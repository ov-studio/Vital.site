...
```lua
local rows = db_query:fetch(count = 0)
```
...
|------|------|-------------|
| `int` | count | Specifies the maximum number of rows to retrieve<br/>When set to `0`, no upper limit is applied and all matching rows are returned |
