...
|-------|------|---------|-------------|
| `type` | `string` | `"VARCHAR(255)"` | SQL column type. Accepts any valid MySQL type, common types include:<br/>• `"INT"` / `"INT UNSIGNED"` / `"BIGINT"`<br/>• `"FLOAT"` / `"DOUBLE"`<br/>• `"BOOLEAN"`<br/>• `"VARCHAR(n)"` / `"TEXT"`<br/>• `"DATETIME"` / `"TIMESTAMP"` |
