...
tagline: "One-line summary of what this resource does" false # Used only by vault
...
| `scripts` | `list<string>` | Yes | None | Sandbox | List of script entries to compile and execute for this resource |
| `files` | `list<string>` | No | `[]` | Sandbox | Glob patterns for non-script assets (models, textures, sounds, etc) |
| `dependencies` | `list<string>` | No | `[]` | Sandbox | Names of other resources that must be running before this one starts |
...
| `tags` | `list<string>` | No | `[]` | Vault | Filters shown on the Vault card - only known tags are kept: `gamemode`, `utility`, `ui`, `physics`, `audio`, `networking`, `tools` |
