...
<Callout type="idea">
  **Prefix the path with `:resourcename` to access directories from another running util.resource.**
...
engine.iprint(util.file.contents("files/images"))
```

```lua lineNumbers title="List the contents of a directory from another running resource"
engine.print("info", "File contents:")
engine.iprint(util.file.contents(":resource_a/files/images"))
```
