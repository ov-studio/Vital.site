...
<Callout type="warn">
  **Both values directly affect client connectivity — change them with care.**
  ...
  - **`port`** — Changing this means clients must connect using the new port, and any firewall or port-forwarding rules must be updated accordingly
...
<Callout type="info">
  **The HTTP server operates independently of the ENet transport layer.**
  ...
  - **Both ports must be reachable** — blocking either one will result in a failed session
  - **Separate firewall rules** — make sure both ports are open and forwarded correctly
</Callout>
...
<Callout type="info">
  **Image asset keys must be registered before use.**
  ...
  - **Register assets** — upload and name your images in the [Discord Developer Portal](https://core.discord.com/developers/applications) under **Rich Presence → Art Assets**
  - **Application ID** — non-numeric or empty values are treated as `0`, and Rich Presence won't initialize
