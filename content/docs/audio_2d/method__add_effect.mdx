...
```lua
local effect_index = self:add_effect(
    type,
    parameters = {}
)
```
...
|------|------|-------------|
| `string` | type | Effect type to add<br/>Refer [**Effect Types**](#effect-types) section |
...
| Type | Name | Description |
|------|------|-------------|
| `int` \| `bool` | effect_index | Index of the newly added effect on success, `false` otherwise |
...
local effect_index = entity:add_effect("reverb", {
    room_size = 0.8,
    wet = 0.5,
    dry = 0.5
})
