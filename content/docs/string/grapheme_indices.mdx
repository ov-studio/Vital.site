...
<Callout type="warn">
grapheme_indices returns byte positions (like codepoint, codes, offset, charpos
and invalidoffset), not character positions. Passing those positions into
util.string.sub or util.string.byte - which now index by character after the
utf8 merge - will give incorrect results except by coincidence on pure-ASCII
input.
</Callout>
