...
| `string` | pattern | The pattern to search for (supports Lua patterns unless plain_text is true) |
| `number` | start_at | Position to start searching from |
| `boolean` | plain_text | If true, pattern is treated as plain text, not a pattern |

### Returns

| Type | Name | Description |
|------|------|-------------|
| `number` | start_at | Starting index of the match, or `nil` if not found |
| `number` | end_at | Ending index of the match, or `nil` if not found |
