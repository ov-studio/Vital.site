...
```lua
local value1, value2, ..., next_pos = string.unpack(format, input, start_at = 1)
```
...
|------|------|-------------|
| `string` | format | Format string (same as used in string.pack) |
| `string` | input | Binary string containing packed data |
| `number` | start_at | Starting position in the string |

### Returns

| Type | Name | Description |
|------|------|-------------|
| `any` | value1 | First unpacked value |
| `any` | value2 | Second unpacked value (if format specifies) |
| `...` | ... | Additional unpacked values |
| `number` | next_pos | Index of the first unread byte in the string |
...
file:close()
local v1, v2, v3 = string.unpack("i4i4i4", data)
engine.print(v1, v2, v3)
```
