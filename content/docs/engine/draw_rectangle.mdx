...
event.on("sandbox:draw", function()
    -- Draw a plain white rectangle, centered at the top section
    engine.draw_rectangle(
        {center[1] - size[1] * 0.5, center[2] - 200},
        size
    )

    -- Draw a filled green rectangle with a black outline, centered in the middle
    engine.draw_rectangle(
    ...
    -- Draw a rectangle rotated 30 degrees around its center, below the previous
    engine.draw_rectangle(
