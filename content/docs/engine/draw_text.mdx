...
| `font` | font | Font userdata to render the text with |
| `int` | font_size | Font size in points |
| `color` | color | Color of the text |
| `alignment[]` | alignment | A two-element table `{horizontal, vertical}` controlling text alignment within the bounding box.<br/>Each value accepts a `string` or a Godot alignment `int`:<br/>• Horizontal: `"left"` `"center"` `"right"` `"fill"` (or `0`–`3`)<br/>• Vertical: `"top"` `"center"` `"bottom"` `"fill"` (or `0`–`3`) |
