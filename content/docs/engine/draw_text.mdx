---
title: engine.draw_text
description: Draws text within a bounding box on the canvas
badge: Client
---

### Syntax

```lua
local status = engine.draw_text(
    text, 
    start_at, 
    end_at, 
    font, 
    font_size, 
    color = {1, 1, 1, 1}, 
    alignment = {"left", "top"}, 
    clip = false, 
    wordwrap = false, 
    stroke = 0, 
    stroke_color = {1, 1, 1, 1}, 
    rotation = 0, 
    pivot = {0, 0}
)
```

<Callout type="warn">
  **Must be called within the `"sandbox:draw"` event.** Invoking this function outside of that event will have no effect.

  - **Inside the event** — executes as expected, rendering text to the canvas each frame
  - **Outside the event** — the call will be silently ignored and nothing will be drawn
  - **Recommended usage** — register a handler via `event.on("sandbox:draw", ...)` and place all canvas draw calls inside it
</Callout>

### Parameters

| Type | Name | Description |
|------|------|-------------|
| `string` | text | Text string to draw |
| `vector2` | start_at | Top-left corner of the bounding box |
| `vector2` | end_at | Bottom-right corner of the bounding box |
| `font` | font | Font to render the text with |
| `int` | font_size | Font size in points |
| `color` | color | Color of the text |
| `alignment[horizontal, vertical]` | alignment | Horizontal alignment: <br/>•`"left"` \| `0` <br/>•`"center"` \| `1` <br/>•`"right"` \| `2` <br/>•`"fill"` \| `3` <br/><br/>Vertical alignment: <br/>•`"top"` \| `0` <br/>•`"center"` \| `1` <br/>•`"bottom"` \| `2` <br/>•`"fill"` \| `3` |
| `bool` | clip | When `true`, text is clipped to the bounding box |
| `bool` | wordwrap | When `true`, text wraps at the bounding box width |
| `int` | stroke | Outline thickness in pixels — set to `0` to disable |
| `color` | stroke_color | Color of the text outline |
| `float` | rotation | Rotation angle in degrees |
| `vector2` | pivot | Pivot point for rotation, relative to `start_at` |

### Returns

| Type | Name | Description |
|------|------|-------------|
| `bool` | status | `true` on successful execution, `false` otherwise |

---

### Examples

```lua lineNumbers title="Draw text with various styles"
local resolution = engine.get_resolution()
local center = {resolution[1] * 0.5, resolution[2] * 0.5}
local my_font = font.create("fonts/Roboto-Regular.ttf")

event.on("sandbox:draw", function()
    -- Draw simple left-aligned text at the top-center
    engine.draw_text(
        "Hello World",
        {center[1] - 150, center[2] - 250},
        {center[1] + 150, center[2] - 200},
        my_font,
        24
    )

    -- Draw centered yellow text in the upper-middle area
    engine.draw_text(
        "Centered",
        {center[1] - 200, center[2] - 150},
        {center[1] + 200, center[2] - 90},
        my_font,
        32,
        {1, 1, 0, 1},
        {"center", "center"}
    )

    -- Draw word-wrapped text clipped to a box on the left side
    engine.draw_text(
        "This is a long string that will wrap inside its bounding box.",
        {center[1] - 400, center[2] - 20},
        {center[1] - 150, center[2] + 180},
        my_font,
        18,
        {1, 1, 1, 1},
        {"left", "top"},
        true,
        true
    )

    -- Draw outlined text on the right side
    engine.draw_text(
        "Outlined",
        {center[1] + 100, center[2] - 20},
        {center[1] + 400, center[2] + 60},
        my_font,
        28,
        {1, 1, 1, 1},
        {"center", "center"},
        false,
        false,
        2,
        {0, 0, 0, 1}
    )

    -- Draw text rotated 15 degrees around its top-left corner
    engine.draw_text(
        "Rotated",
        {center[1] - 100, center[2] + 120},
        {center[1] + 100, center[2] + 170},
        my_font,
        20,
        {1, 1, 1, 1},
        {"left", "top"},
        false,
        false,
        0,
        {1, 1, 1, 1},
        15,
        {0, 0}
    )
end)
```
