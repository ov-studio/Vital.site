...
<Callout type="warn">
  **Must be called within the `vital.sandbox:draw` event.** Invoking this function outside of that event will have no effect.
  ...
  - **Recommended usage** — register a handler via `network:fetch("sandbox:draw", true):on(...)` and place all canvas draw calls inside it
</Callout>
...
network:fetch("sandbox:draw", true):on(function()
