...
<Callout type="info">
  **Bootstrap runs once at startup, after the resource scan completes.**
  ...
  - **Order is preserved** — resources are started top-to-bottom as listed.
  - **Dependencies are resolved automatically** — a bootstrap resource's dependencies are started before it, even if they are not listed here.
  - **Each entry must be a valid resource name** — it must match a folder under `resources/` and pass the resource name validation rules (`a-z`, `0-9`, `_` only).
  - **Already-running resources are skipped** — no error is raised if a listed resource is already running by the time bootstrap executes.
</Callout>
