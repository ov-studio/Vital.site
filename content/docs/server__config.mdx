...
description: Configure a Vital.sandbox server — its identity, ports, bootstrapper, socials, and master-list — via config.yaml
---
...
<Callout type="info">
  **Where the config lives**
  ...
  - **Single file** — all sections (`server`, `network`, `http`, `bootstrap`, `social`, `masterlist`) live in this one file.
...
Controls whether this server is listed on the public Vital.sandbox master-list and how often it reports in.
...
<Callout type="info">
  **Heartbeat timing**
  ...
  The master-list expires listings after a TTL (currently **660 s / 11 minutes**). Set `interval` comfortably below that value — the default of `300 s` provides a healthy margin even if a single heartbeat is missed.
</Callout>

| Key | Type | Default | Description |
|---|---|---|---|
| `enabled` | `boolean` | `true` | When `true`, the server reports itself to the public master-list |
| `token` | `string` | `""` | API token obtained from a staff member via Discord |
| `url` | `string` | `"https://vital-sandbox.com/api/masterlist"` | Base URL of the master-list API |
| `interval` | `integer` | `300` | How often (in seconds) the server sends a heartbeat to the master-list |