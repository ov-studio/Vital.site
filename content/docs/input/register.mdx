...
<Callout type="info">
  **Handlers are scoped to the environment that registered them and are automatically cleaned up when that environment is destroyed.**
  ...
  - **Console fallback** — typing an unrecognized command into the console dispatches it here automatically;<br/>there's no need to call [`util.input.execute`](/docs/input/execute) yourself just to support console input
</Callout>
