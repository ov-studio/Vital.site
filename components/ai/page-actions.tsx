...
const AIProviders = [
  {
    title: 'GitHub',
    url: (githubUrl: string) => githubUrl,
    icon: (
  ...
  {
    title: 'Scira AI',
    url: (_: string, q: string) => `https://scira.ai/?${new URLSearchParams({ q })}`,
    icon: (
      <svg viewBox="0 0 910 934" fill="none">
  ...
  {
    title: 'ChatGPT',
    url: (_: string, q: string) => `https://chatgpt.com/?${new URLSearchParams({ hints: 'search', q })}`,
    icon: (
  ...
  {
    title: 'Claude',
    url: (_: string, q: string) => `https://claude.ai/new?${new URLSearchParams({ q })}`,
    icon: (
      <svg fill="currentColor" viewBox="0 0 24 24">
...
export function ViewOptions({ markdownUrl, githubUrl }: { markdownUrl: string; githubUrl: string }) {
  const items = useMemo(() => {
    ...
    return AIProviders.map(provider => ({ title: provider.title, href: provider.url(githubUrl, query), icon: provider.icon }));
